...
class CmcicCleaner extends BaseTask
{
  ...
  private function deleteRecurrent()
  {
    ...
    foreach ($this->ident_paiements as $ident_paiement) {
      // On vérifie s'il ne s'agit pas d'une exception
      if ($this->getCmcic()->checkIfItsNotExceptionBeforeCancel($ident_paiement, Config::get('cmcic_nb_month')) === true) {
        $this->logSection(__METHOD__, 'Traitement du donateur : '.$ident_paiement.' : exception de date et nb occurence');
        continue;
      }
